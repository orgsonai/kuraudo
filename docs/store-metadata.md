...
## Microsoft Store（提出未着手 — Windowsビルドは可能）
...
• 選べる同期方式 — Google Drive / WebDAV（Nextcloud等） / ローカルパス（NAS等）から選択可能
• ローカル・ファースト — オフラインでも完全に動作
• クロスプラットフォーム — Linux / Android / Windows
...
• Choose your sync — Google Drive / WebDAV (Nextcloud, Synology, etc.) / Local Path (NAS, external drive)
• Local-first — works fully offline
• Cross-platform — Linux / Android / Windows
...
- **同期（任意機能）**: ユーザーが明示的に有効化した同期方式（Google Drive / WebDAV / ローカルパス）に対し、暗号化済みファイルのみを送信。マスターパスワードは送信されない
...
### Snap Store
- **名前**: kuraudo
- **概要**: Open-source password manager with flexible sync (Google Drive / WebDAV / Local Path)
- **ライセンス**: GPL-3.0
- **confinement**: strict
...
Type=Application
Name=Kuraudo
Comment=Password Manager with Flexible Sync (Google Drive / WebDAV / Local)
Exec=kuraudo
Icon=kuraudo
