...
- **設定もデータと同じ場所に** — 設定ファイル・バックアップはVaultと同じフォルダに配置（OS標準のアプリ設定フォルダに置くのは、前回開いたVaultの場所など最小限の情報だけ）
...
セキュリティ上の問題を発見した場合は、**公開Issueではなく** [SECURITY.md](docs/SECURITY.md) に記載の方法でご報告ください。
...
Issue報告は歓迎します。PRについては [CONTRIBUTING.md](docs/CONTRIBUTING.md) をご覧ください。
