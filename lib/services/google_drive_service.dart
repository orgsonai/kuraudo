...
class GoogleDriveService implements SyncBackend {
  ...
  // OAuth2 設定（デスクトップ用）
  // Windows/Linux/macOS用の「デスクトップ アプリ」クライアントID（公開情報）
  static const String _clientId =
      '957494358901-n8b26mltvgqbom0fea35ame5gi2js5v7.apps.googleusercontent.com';
  // レガシークライアントとの互換用。デスクトップアプリクライアントでは不要。
  ...
  Future<void> _saveToken() async {
    try {
      await _secureStorage.write(
          key: '${_tokenKeyPrefix}access_token', value: _accessToken);
      await _secureStorage.write(
          key: '${_tokenKeyPrefix}refresh_token', value: _refreshToken);
      await _secureStorage.write(key: '${_tokenKeyPrefix}email', value: _email);
      await _secureStorage.write(
          key: '${_tokenKeyPrefix}expiry',
  ...
  Future<bool> _loadToken() async {
    try {
      _accessToken =
          await _secureStorage.read(key: '${_tokenKeyPrefix}access_token');
      _refreshToken =
          await _secureStorage.read(key: '${_tokenKeyPrefix}refresh_token');
      _email = await _secureStorage.read(key: '${_tokenKeyPrefix}email');
      final expiryStr =
          await _secureStorage.read(key: '${_tokenKeyPrefix}expiry');
  ...
  Future<bool> _refreshAccessToken() async {
    ...
    try {
      ...
      if (_clientSecret.isNotEmpty) body['client_secret'] = _clientSecret;
      final response = await http.post(Uri.parse(_tokenEndpoint), body: body);
  ...
  Future<bool> _signInDesktop() async {
    try {
      ...
      // RFC 7636 PKCE: デスクトップアプリは秘密情報を安全に保持できないため、
      // 一時的なverifierで認証コードを保護する。
      final codeVerifier = base64Url
          .encode(List<int>.generate(64, (_) => Random.secure().nextInt(256)))
          .replaceAll('=', '');
      final codeChallenge = base64Url
          .encode(SHA256Digest()
              .process(Uint8List.fromList(utf8.encode(codeVerifier))))
          .replaceAll('=', '');
      ...
      try {
        final request =
            await server.first.timeout(const Duration(seconds: 120));
        ...
        if (receivedState == null || receivedState != stateNonce) {
          ...
          request.response
            ...
            ..write(
                '<html><body style="font-family:sans-serif;text-align:center;padding:60px;background:#0a0a0b;color:#e4e4e7;">'
      ...
      if (authCode == null) {
        _lastAuthError = '認証コードを受信できませんでした';
        _status = SyncStatus.notSignedIn;
        return false;
      }
      ...
      if (_clientSecret.isNotEmpty) tokenBody['client_secret'] = _clientSecret;
      final tokenResponse =
          await http.post(Uri.parse(_tokenEndpoint), body: tokenBody);

      if (tokenResponse.statusCode != 200) {
        _lastAuthError = _oauthErrorMessage(tokenResponse);
        _status = SyncStatus.error;
        return false;
      }
  ...
  String _oauthErrorMessage(http.Response response) {
    try {
      final body = jsonDecode(response.body) as Map<String, dynamic>;
      final description = body['error_description'] as String?;
      final error = body['error'] as String?;
      return description ?? error ?? 'HTTP ${response.statusCode}';
    } catch (_) {
  ...
  Future<SyncResult> upload(Uint8List fileBytes) async {
    if (_driveApi == null)
      return SyncResult(
          status: SyncStatus.notSignedIn, message: 'Googleアカウントにサインインしてください');
    if (!await _ensureValidToken())
      return SyncResult(status: SyncStatus.error, message: 'トークンの更新に失敗しました');
    _setupDriveApi();
    ...
    try {
      ...
      if (existingFile != null && existingFile.id != null) {
        ...
        await _driveApi!.files
            .update(updatedFile, existingFile.id!, uploadMedia: media);
      } else {
  ...
  Future<Uint8List?> download() async {
    ...
    try {
      ...
      final media = await _driveApi!.files.get(remoteFile.id!,
  ...
  Future<SyncResult> smartSync({
    ...
    try {
      ...
      final localChanged =
          localModifiedAt.isAfter(lastSync.add(const Duration(seconds: 3)));
      final remoteChanged =
          remoteModifiedAt.isAfter(lastSync.add(const Duration(seconds: 3)));
      ...
      if (localChanged && !remoteChanged) {
        ...
        return SyncResult(
            status: SyncStatus.success,
            message: 'クラウドに新しいデータがあります（マージ同期を推奨）',
  ...
  @override
  Future<SyncResult> createBackup(Uint8List fileBytes,
      {String label = 'manual'}) async {
    if (_driveApi == null)
      return SyncResult(status: SyncStatus.notSignedIn, message: 'サインインしてください');
    if (!await _ensureValidToken())
      return SyncResult(status: SyncStatus.error, message: 'トークン更新失敗');
    _setupDriveApi();
  ...
  Future<void> _pruneBackups(String label) async {
    ...
    try {
      final spaces = _useAppDataFolder ? 'appDataFolder' : 'drive';
      final query =
          "name contains '$_backupPrefix${label}_' and trashed = false";
      final result = await _driveApi!.files.list(
          q: query,
          spaces: spaces,
  ...
  Future<SyncResult> sync(
      {required Uint8List localFileBytes,
      required DateTime localModifiedAt}) async {
    return smartSync(
        localFileBytes: localFileBytes, localModifiedAt: localModifiedAt);
  }
  ...
  Future<bool> deleteRemoteFile() async {
    ...
    try {
      ...
      if (remoteFile?.id != null) {
        await _driveApi!.files.delete(remoteFile!.id!);
        return true;
      }
      return false;
    } catch (_) {
